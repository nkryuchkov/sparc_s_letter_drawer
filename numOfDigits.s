 	.global numOfDigits		! Declare the symbol to be globally visible
...
numOfDigits:
	...
	smul %l0, %l3, %l0		! multiply num by %l3 with sign to extend the
					! sign
					! in %y register because sdiv performs division 					! of register pair where %y register represents 
...
numOfDigitsLoop:
	...
	sdiv %l0, %l1, %l0		! divide register pair %y:l0 (num with
					! its sign ! extension) by %l1 (base) and set %l0 to 
					! quotient

	ba numOfDigitsLoop		! do a loop
	add %l2, 0, %i0			! incrementing %l2 by 1

numOfDigitsExit:
	ret				! return from subroutine
	restore				! move the register window backward
