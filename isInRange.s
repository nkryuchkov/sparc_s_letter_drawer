 	.global isInRange		! Declare the symbol to be globally visible
...
isInRange:
	...
	subcc %i1, %i0, %l0	! subtract minRange from maxRange setting flag
				! to check if minRange is larger than maxRange
				! we save the result in %l0 to check range for
				! the case if minRange=maxRange and
				! exclusive!=0 
	
	bl,a isInRangeExit	! if minRange-maxRange < 0
				! (i.e. minRange is larger) than we perform an
				! annulled branch to the isInRangeExit label
	
	sub %g0, 1, %i0		! annulled branch means this instuction 
				! in the delay slot will be executed only if
				! the  branch is taken. We set the  
				! subroutine's return val to -1
	
	subcc %l0, %i3, %g0	! minRange-maxRange-(exclusive flag) < 0?
	bl,a isInRangeExit	! then go to isInRangeExit
	sub %g0, 0, %i0		! and set subroutine's return value to 0 and 
				! branch 
	subcc %i3, 0, %i3	! if exclusive!=0
	bne,a isInRangeLabel	! then
	add %g0, 1, %i3		! we set to exclusive to 1

isInRangeLabel:			! branch is taken here direct after its delay 				
				! slot
